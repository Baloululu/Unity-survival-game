...
public static class MeshGenerator {
	...
	public static MeshData GenerateTerrainMesh(float[,] heightMap, float heightMultiplier, AnimationCurve _heightCurve, int levelOfDetail)
    {
        ...
        int meshSizeUnsimplifier = borderedSize - 2;
        float topLeftX = (meshSizeUnsimplifier - 1) / -2f;
        float topLeftZ = (meshSizeUnsimplifier - 1) / 2f;
        ...
        for (int y = 0; y < borderedSize; y += meshSimplicationIncrement)
        {
            for (int x = 0; x < borderedSize; x += meshSimplicationIncrement)
            {
                ...
                {
                    vertexIndicesMap[x, y] = meshVerticeIndex;
                    meshVerticeIndex++;
                }
            }
        }
        ...
        for (int y = 0; y < borderedSize; y += meshSimplicationIncrement)
        {
            for (int x = 0; x < borderedSize; x += meshSimplicationIncrement)
            {
                ...
                Vector3 vertexPosition = new Vector3(topLeftX + percent.x * meshSizeUnsimplifier, height, topLeftZ - percent.y * meshSizeUnsimplifier);
...
public class MeshData
{
    ...
    Vector3[] CalculateNormals()
    {
        ...
        for (int i = 0; i < triangleCount; i++)
        {
            ...
            Vector3 triangleNormal = SurfaceNormaleFromVertices(vertexIndexA, vertexIndexB, vertexIndexC);
    ...
    Vector3 SurfaceNormaleFromVertices(int indexA, int indexB, int indexC)
    {
